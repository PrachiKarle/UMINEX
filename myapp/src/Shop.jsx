    ...
    for(let x of acc.data){
       if(x.Login){
        ...
        await axios.patch(`http://localhost:3000/account/${x.id}`, {
          BuyDetail:[...acc.BuyDetail,product.data],
          buyCost:acc.buyCost+product.price,
        })
       }
    }
    if(!flag){
            ...
            return (
              <>
                <div className="col-lg-3 col-md-6 col-12 p-4">
                  <div className="h-100 w-100 py-3" style={{ backgroundColor: "white" }}>
                    <div>
                      <img src={val.img} className="img-fluid" alt="" />
                    </div>
                    ...
                    <button className="btn btn-danger my-2" style={{ marginLeft: "25%" }} onClick={() => add(val.id)}>Shop Now</button>
                  </div>
                </div>
              </>
            )
